        ...
        <MainLayout>
            <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '3rem 1.5rem' }}>
                <div style={{ marginBottom: '2rem' }}>
                    ...
                    <h1 style={{ fontFamily: 'var(--font-body)', fontSize: '2rem', fontWeight: 800, color: 'white', textTransform: 'uppercase' }}>
                        Checkout
                    </h1>
                </div>
                ...
                <form onSubmit={submit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '2rem', alignItems: 'start' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div style={{ backgroundColor: 'var(--color-surface)', padding: '2rem', border: '1px solid var(--color-border)' }}>
                                <h2 style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.15em', color: 'white', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
                                    Shipping Information
                                </h2>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    {fields.map((field) => (
                                        <div key={field.key} style={{ gridColumn: field.col === 2 ? 'span 2' : 'span 1' }}>
                                            <label style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', color: 'var(--color-muted)', textTransform: 'uppercase', display: 'block', marginBottom: '0.4rem' }}>
                                                {field.label}
                                            </label>
                                            <input
                                                type="text"
                                            ...
                                            />
                                            {errors[field.key] && <p className="error-message">{errors[field.key]}</p>}
                                        </div>
                                    ))}
                            ...
                            <div style={{ backgroundColor: 'var(--color-surface)', padding: '2rem', border: '1px solid var(--color-border)' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                    {[
                                        ...
                                        >
                                            <input type="radio" name="payment_method" value={val} checked={data.payment_method === val} onChange={() => setData('payment_method', val)} style={{ display: 'none' }} />
                                            <div style={{ width: 18, height: 18, border: `2px solid ${data.payment_method === val ? 'var(--color-accent)' : 'var(--color-border)'}`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'border-color 0.2s' }}>
                                                {data.payment_method === val && <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--color-accent)' }} />}
                                            </div>
                                ...
                                <div style={{ marginTop: '1rem', padding: '0.75rem', backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid var(--color-border)', fontSize: '0.75rem', color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <div style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: '1.5rem', position: 'sticky', top: '80px' }}>
                            <h3 style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.15em', color: 'white', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
                                Order Summary
                            </h3>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <span style={{ color: 'var(--color-accent)', fontWeight: 700, fontSize: '1.1rem' }}>{formatPrice(total)}</span>
                                </div>
                            </div>

                            <button type="submit" disabled={processing} className="btn-primary" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: processing ? 0.7 : 1, cursor: processing ? 'not-allowed' : 'pointer' }}>
                                <Check size={16} />
